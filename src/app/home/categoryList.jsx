...
export default async function CategoryList({categories}) {
    ...
    const listCategories = categories.map(category => <li key={category.category_id} className="hover:bg-gray-200"><div className=""><h2>{category.name}:</h2> {category.current_spend}/{category.max_spend}<hr></hr></div></li>);
